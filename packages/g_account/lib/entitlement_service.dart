...
///
/// ## NO METHOD ON THIS CLASS MAY THROW
///
/// It became the top crash in the field, fatal, fifty events across nine users
/// in two days, and it took crash-free users to 88 percent:
///
///     PlatformException(UNAVAILABLE, BillingClient is unset. Try reconnecting.)
///       ._extractReplyValueOrThrow (messages.g.dart:26)
///       InAppPurchaseApi.isReady   (messages.g.dart:1616)
///       EntitlementService.start   (entitlement_service.dart:142)
///
/// Every other platform call here was already wrapped. `isAvailable` was not,
/// in [start] and again in [buy], and it is the FIRST call either of them makes.
///
/// ─── WHY IT FIRES HERE AND NOT IN AN ORDINARY APP ───────────────────────────
///
/// `in_app_purchase_android` builds its `BillingClient` when the plugin attaches
/// to an ACTIVITY. G Launcher warms its Flutter engine in
/// `LauncherApplication.onCreate` (see `LauncherHostApiImpl`, which says so in
/// its own header), so Dart `main` and the app root run in a process where no
/// Activity exists yet. Any billing call landing in that window asks a client
/// that has not been constructed, and the plugin's Pigeon wrapper turns the
/// answer into an exception rather than a false.
///
/// A launcher makes that window recur constantly. `main.dart` records six
/// LOW_MEMORY kills in fourteen hours; every one of them is another cold start
/// racing the Activity attach.
///
/// ─── AND WHY IT STOPPED PEOPLE BUYING ───────────────────────────────────────
///
/// The [buy] copy of the probe is the one users met. Tap Get, `isAvailable`
/// throws inside an async handler nothing was catching, and the launcher dies.
/// The report reaching us was "I cannot buy distros", which is exactly what that
/// looks like from the outside.
///
/// So the contract is now explicit: **nothing here throws.** A store that will
/// not answer degrades to "unavailable, nothing owned", the same call
/// `main.dart` makes about Firebase one layer down. Entitlements are optional;
/// the launcher is not.
class EntitlementService {
  ...
  /// The backoff for re-probing after the store refused to answer.
  ///
  /// BOUNDED, AND SHORT. The failure this exists for is a race against the
  /// Activity attach, measured in hundreds of milliseconds, so three tries
  /// inside twenty-two seconds covers it comfortably. What it must NOT do is
  /// keep asking forever: on a de-Googled ROM or a device without Play Services
  /// the answer is a permanent, correct false, and a service that polls it is
  /// spending that user's battery to be told the same thing.
  static const _probeBackoff = <Duration>[
    Duration(seconds: 2),
    Duration(seconds: 5),
    Duration(seconds: 15),
  ];
  ...
  /// Every SKU any caller has asked about, so a retry does not have to be told
  /// again. A retry that dropped the set would reconnect and then query
  /// nothing, which looks like success and leaves the storefront priceless.
  ...
  Future<void> start(Set<String> skus) async {
    ...
    if (!_connected) {
      _available = await _probeAvailable();
      if (_disposed) return;
      _connected = _available;

      // NOT just a catch. Swallowing the throw alone would turn a fatal crash
      // into a storefront that is permanently empty for anyone whose first
      // start lost the race, because nothing else re-asks: the caller only
      // calls `start` again when the catalogue lands, which on a warm cache is
      // never. The retry is what actually restores the sale.
      if (!_available) _scheduleRetry();
    }
  ...
  Future<bool> buy(String sku) async {
    ...
    //
    // ─── RETRIED IN LINE, UNLIKE start's ──────────────────────────────────
    //
    // A tap PROVES an Activity is on screen, so "BillingClient is unset" at this
    // exact moment is a race that has already resolved or is about to. Waiting
    // out a second of it is far better than the alternative the user actually
    // met, and better than the honest-looking refusal that preceded it.
    //
    // Quiet until the last attempt: three failures are one failure, and three
    // messages about it is the storefront shouting.
    if (!_connected) {
      const waits = [Duration.zero, Duration(milliseconds: 300), Duration(milliseconds: 900)];
      for (var i = 0; i < waits.length; i++) {
        if (waits[i] > Duration.zero) await Future<void>.delayed(waits[i]);
        if (_disposed) return false;
        _available = await _probeAvailable(quiet: i < waits.length - 1);
        if (_disposed) return false;
        if (_available) break;
      }
      _connected = _available;
    ...
    // nothing.
    //
    // Wrapped for the same reason as everything else here now. This one is the
    // very last thing between a person and a purchase, and a throw at this
    // point takes the launcher down while their money is on screen.
    try {
  ...
  /// `isAvailable`, which is allowed to throw, made into one that is not.
  ///
  /// EVERY caller goes through this. The two that did not are what produced the
  /// crash in the class header, and both of them were the first platform call
  /// their method made, which is why the failure was total rather than partial.
  ///
  /// [quiet] suppresses the message while a caller is mid-retry.
  Future<bool> _probeAvailable({bool quiet = false}) async {
    if (_disposed) return false;
    try {
      return await _iap.isAvailable();
    } catch (e) {
      // PlatformException(UNAVAILABLE, BillingClient is unset) lands here, and
      // so does every other way the platform can decline to answer. They all
      // mean the same thing to this class: no store right now.
      if (!quiet) _emitError('Store unavailable: $e');
      return false;
    }
  }

  /// Re-probe later, a bounded number of times.
  ///
  /// Re-enters [start] rather than only flipping a flag, because connecting is
  /// not the point: the products and the restore are, and both live in there.
  /// [_wanted] carries the SKU set so the retry does not reconnect and then ask
  /// about nothing.
  void _scheduleRetry() {
    if (_disposed || _connected) return;
    if (_probeAttempts >= _probeBackoff.length) return;

    final delay = _probeBackoff[_probeAttempts];
    _probeAttempts++;

    _retryTimer?.cancel();
    _retryTimer = Timer(delay, () {
      if (_disposed || _connected) return;
      // Unawaited on purpose: this is a timer callback and there is nobody to
      // return to. `start` cannot throw, so there is nothing to lose here
      // either.
      start(Set<String>.from(_wanted));
    });
  }
  ...
  Future<void> _handle(List<PurchaseDetails> purchases) async {
    ...
    for (final purchase in purchases) {
      ...
      if (purchase.pendingCompletePurchase) {
        // WRAPPED, AND THE PARAGRAPH ABOVE STILL STANDS. `_handle` is an async
        // listener callback: a throw in here does not reach the `onError` on
        // the subscription, it becomes an unhandled async error and kills the
        // process, which is the same shape as the crash in the class header.
        //
        // Catching does not abandon the acknowledgement. A purchase that fails
        // to acknowledge stays `pendingCompletePurchase` on Play's side and is
        // redelivered through this same stream on the next `restore`, so the
        // three-day clock gets further attempts. Crashing here would have
        // stopped the rest of the batch being acknowledged at all.
        try {
          await _iap.completePurchase(purchase);
        } catch (e) {
          _emitError('Could not acknowledge a purchase: $e');
        }
      }
