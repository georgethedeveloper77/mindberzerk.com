...
interface LauncherHostApi {
   ...
   */
  fun openMotionWallpaper(path: String, callback: (Result<Boolean>) -> Unit)
  /**
   ...
   */
  fun motionWallpaperActive(callback: (Result<Boolean>) -> Unit)
  fun setWallpaper(source: String, target: String, fit: String, letterboxColor: Long, focalX: Double, focalY: Double, zoom: Double, callback: (Result<Boolean>) -> Unit)
  /**
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: LauncherHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.g_launcher.LauncherHostApi.openMotionWallpaper$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pathArg = args[0] as String
            api.openMotionWallpaper(pathArg) { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(LauncherApiPigeonUtils.wrapError(error))
              } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.g_launcher.LauncherHostApi.motionWallpaperActive$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            api.motionWallpaperActive{ result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(LauncherApiPigeonUtils.wrapError(error))
              } else {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val targetArg = args[1] as String
            ...
            api.setWallpaper(sourceArg, targetArg, fitArg, letterboxColorArg, focalXArg, focalYArg, zoomArg) { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(LauncherApiPigeonUtils.wrapError(error))
              } else {
