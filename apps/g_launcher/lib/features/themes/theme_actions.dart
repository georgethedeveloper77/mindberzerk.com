...
Future<void> _download(
  ...
  switch (result.status) {
    ...
    case 'notEntitled':
      context.showMessage(
          context.t('themes.needsPurchaseFirst', {'name': card.name}));
    case 'appTooOld':
      context.showMessage('${card.name} needs a newer version of G Launcher');
    case 'noSpace':
      context.showMessage(
          context.t('themes.notEnoughFreeSpace', {'name': card.name}));
    case 'cancelled':
      break;
    case 'rejected':
      ...
      context.showMessage(
          context.t('themes.failedVerification', {'name': card.name}));
    default:
      context.showMessage(
          context.t('themes.couldNotDownload', {'name': card.name}));
  }
}
...
Future<void> runThemeCardAction(
  ...
  if (_themeCardIsActiveNow(ref, card)) {
    ...
    context.showMessage(
        context.t('themes.isYourCurrentDistro', {'name': card.name}));
    return;
  }
  ...
  switch (card.status) {
    ...
    case CardStatus.locked:
      ...
      if (!started && context.mounted) {
        ...
        // rather than nothing.
        context.showMessage(
            context.t('themes.notAvailableToBuy', {'name': card.name}));
      }
