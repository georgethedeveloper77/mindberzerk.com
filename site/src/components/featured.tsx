  ...
  return (
    ...
    >
      ...
      <div className="absolute left-3 top-9 text-white">
        <div className="font-display text-2xl font-semibold leading-none">12:04</div>
...
function TintScene({ app }: { app: AppMeta }) {
  return (
    ...
    >
      <span className="grid size-16 place-items-center rounded-[18px] bg-white/15 font-display text-2xl font-bold">
        {app.mark}
      </span>
...
export function Featured({ apps }: { apps: AppMeta[] }) {
  ...
  return (
    <div>
      <div className="rounded-[30px] border border-line bg-card p-7 pb-6 shadow-lift">
        <div className="grid min-h-[356px] items-center gap-6 max-[620px]:justify-items-center max-[620px]:text-center min-[621px]:grid-cols-[1fr_176px]">
          <div className="flex flex-col gap-3.5 max-[620px]:items-center">
            <span className="self-start rounded-full bg-accent-soft px-3 py-1 text-[11.5px] font-bold uppercase tracking-wider text-accent-deep max-[620px]:self-center">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {store ? (
                <a
                  href={store}
                  className="rounded-full border-[1.5px] border-line bg-white px-3.5 py-[7px] text-[12.5px] font-semibold text-ink transition hover:border-[#cfc7de]"
                >
                  Google Play
                </a>
              ) : isComing(app.state) ? (
                <span className="rounded-full border-[1.5px] border-dashed border-line px-3.5 py-[7px] text-[12.5px] font-semibold text-ink-3">
                  Coming soon
                </span>
              ) : null}
            </div>
          </div>
          <div className="w-[176px] rounded-[28px] bg-[#17121f] p-[7px] shadow-deep">
            ...
            <button
              ...
              className={`rounded-full border-[1.5px] px-[15px] py-[7px] text-[12.5px] font-semibold transition ${
                i === idx
                  ...
                  : 'border-line bg-page text-ink-3 hover:text-ink'
              }`}
            >
              {a.name.split(':')[0]}
            </button>
      ...
      <p className="mt-3 text-center text-xs font-medium text-ink-3">
