...
const inputCls =
  'w-full rounded-xl border-[1.5px] border-line bg-white px-3.5 py-2.5 text-[14px] text-ink placeholder:text-ink-3 focus:border-accent focus:outline-none';
...
export function Contact() {
  ...
  return (
    <div className="grid overflow-hidden rounded-[28px] border border-line bg-card shadow-soft lg:grid-cols-[0.9fr_1.1fr]">
      <div
        className="flex flex-col justify-center p-8 sm:p-12"
        style={{
          background:
            'radial-gradient(560px 340px at 0% 0%, var(--color-lav) 0%, transparent 65%), radial-gradient(480px 320px at 100% 100%, var(--color-peach) 0%, transparent 62%), #fff',
        }}
      >
        ...
        <a href={`mailto:${CONTACT_EMAIL}`} className="mt-5 inline-flex items-center gap-2 text-[15px] font-bold text-accent transition hover:text-accent-deep">
          {CONTACT_EMAIL}
        </a>
      </div>
      ...
      <form onSubmit={submit} className="flex flex-col gap-3.5 p-8 sm:p-12">
        ...
        <label className="block">
          <span className="mb-1.5 block text-[13px] font-semibold text-ink">What is this about</span>
        ...
        <div className="mt-1 flex flex-wrap items-center gap-4">
          <button
            ...
            className="rounded-full bg-accent px-6 py-3 text-[15px] font-semibold text-white shadow-[0_12px_28px_rgba(109,74,232,0.32)] transition hover:-translate-y-px hover:bg-accent-deep disabled:opacity-60 disabled:hover:translate-y-0"
          >
            {status === 'sending' ? 'Sending' : 'Send message'}
          </button>
          {status === 'sent' && <span className="text-[13.5px] font-semibold text-ok">Sent. We will get back to you.</span>}
          {status === 'failed' && <span className="text-[13.5px] font-semibold text-plan">{error}</span>}
          {status === 'unconfigured' && (
            <span className="text-[13.5px] font-medium text-ink-3">
              The form is not wired up yet. Email us at{' '}
              <a className="font-bold text-accent" href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a>
            </span>
          )}
        </div>
      </form>
